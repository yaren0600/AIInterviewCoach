...
/// <summary>
/// Bu controller, kullanıcının AI gelişim planını almak,
/// görevleri tamamlandı/tamamlanmadı yapmak ve yeni plan oluşturmak
/// için gerekli endpointleri sağlar.
/// Kullanıcı kimliği JWT token'ından alınır.
/// </summary>
...
public class StudyPlanController : ControllerBase
{
    ...
    [HttpPost("regenerate")]
    public async Task<IActionResult> RegenerateStudyPlan()
    {
        var userIdClaim = User.FindFirst(ClaimTypes.NameIdentifier)?.Value;
        ...
        var result = await _studyPlanService.RegenerateStudyPlanAsync(userId);

        return Ok(new ApiResponseDto<StudyPlanDto>
        {
            Success = true,
            Message = "Yeni AI gelişim planı başarıyla oluşturuldu.",
            Data = result
        });
    }
