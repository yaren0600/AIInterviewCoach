...
public class StudyPlanService : IStudyPlanService
{
	...
	public async Task<StudyPlanDto> GetMyStudyPlanAsync(int userId)
	{
		...
		var technicalFocusTopics = GetTechnicalFocusTopics(categoryPerformances.Select(x => x.Category).ToList());
		var communicationFocusTopics = GetCommunicationFocusTopics(categoryPerformances.Select(x => x.Category).ToList());
		var recommendedPracticeModes = GetRecommendedPracticeModes(categoryPerformances.Select(x => x.Category).ToList());
		...
		return new StudyPlanDto
		{
			...
			WeeklyPlan = GenerateWeeklyPlan(weakestCategory, recommendedPracticeModes)
		};
	}
	...
	private StudyPlanDto CreateEmptyStudyPlan()
	{
		return new StudyPlanDto
		{
			GeneralSummary = "Henüz yeterli mülakat cevabı olmadığı için kişisel gelişim planı oluşturulamadı. İlk planın için en az bir mülakat oturumunu tamamlamalısın.",
			...
			WeeklyPlan = new List<WeeklyStudyPlanItemDto>
			{
				...
				new()
				{
					Day = "2. Gün",
					Focus = "CV anlatımı",
					Task = "CV’ndeki bir projeyi 2 dakika içinde anlatma pratiği yap.",
